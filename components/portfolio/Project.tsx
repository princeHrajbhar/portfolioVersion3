...
const Project: React.FC<ProjectProps> = ({ id }) => {
  ...
  const getFloatingIconPositions = () => {
    ...
    return techIcons.map((tech, i) => {
      const seed = i * 97.7;
      return {
        ...tech,
        left: `${Math.abs(Math.sin(seed) * 50 + 25)}%`,
        top: `${Math.abs(Math.cos(seed * 0.7) * 50 + 25)}%`,
        delay: i * 2,
        id: i
      };
    });
  ...
  const FloatingTechIcon = ({ icon, delay, left, top }: { icon: React.ReactNode; delay: number; left: string; top: string }) => (
    <motion.div
    ...
    <section id={id} className="">
      ...
      >
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {floatingIconPositions.map((tech) => (
            <FloatingTechIcon 
              key={`bg-tech-${tech.id}`} 
              icon={tech.icon} 
              delay={tech.delay} 
